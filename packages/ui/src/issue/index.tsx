...
export const Issue: Component<Props> = (props) => {
  const issueURL = (scope: Scope, query: string, errorMessage: string) => {
    const body = `# Scope
...
`;
    ...
    return `https://github.com/Internet-Society-Belgium/isTrust/issues/new?labels=bug&body=${encodedBody}`;
  };

  return (
    <div class="flex flex-col items-center justify-center gap-2 py-4">
      <p>{props.error.message}</p>
      <Show when={props.error.name !== "UserError"}>
        <a
          ...
          href={issueURL(props.scope, props.query, props.error.message)}
        >
          Report
          <svg
